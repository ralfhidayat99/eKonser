...
@section('title', 'Welcome to eKonser')

@section('content')
<h1 class="mb-3">Welcome to eKonser</h1>
<p class="lead">Discover amazing concerts happening near you!</p>

<!-- Secret Admin Login Button -->
<a href="{{ route('login') }}" class="btn btn-link p-0" style="opacity: 0.1; position: fixed; bottom: 10px; right: 10px;" title="Admin Login">
    &#128274;
</a>
...
<div class="concert-list d-flex flex-wrap gap-3">
    ...
    <a href="{{ route('concerts.show', $concert) }}" class="card text-decoration-none text-dark" style="flex: 1 1 24rem; min-width: 18rem; max-width: 100%;">
        ...
        <div class="card-body p-2">
            ...
            <p class="card-text mb-1"><strong>Artist:</strong> {{ $concert->artist }}</p>
            <p class="card-text mb-1"><strong>Date:</strong> {{ \Carbon\Carbon::parse($concert->date)->format('F d, Y') }}</p>
            <p class="card-text mb-1"><strong>Location:</strong> {{ $concert->location }}</p>
        </div>
    </a>
    @endforeach
</div>
